...
NOTE: `useREST` é fornecido como uma **prévia técnica** recomendada para ambientes de teste e não para cargas de trabalho de produção. Quando definido como `true`, o Astra Trident usará as APIS REST do ONTAP para se comunicar com o back-end. Esse recurso requer o ONTAP 9.8 e posterior. Além disso, a função de login do ONTAP usada deve ter acesso ao `ontap` aplicativo. Isso é satisfeito com as funções e `cluster-admin` predefinidas `vsadmin`.
...
"Versão": 1, "storageDriverName": "ONTAP-nas-FlexGroup", "managementLIF": "10,0.0,1", "dataLIF": "10,0.0,2", "labels": "k8scluster": "Test-cluster-East-1b", "backend": "test1-ONTAP-cluster", "svm": "SECREADE", "sECREME"
